...
When using the merge operation to merge data with a Delta table, it assumes that the schema of the incoming data (additionalData in your case) matches the schema of the Delta table (oldData). If there are new columns in the incoming data that are not present in the Delta table, they will be ignored during the merge operation.
